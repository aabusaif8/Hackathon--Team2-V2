...
function notTuesdayCheck(req, res, next) {
...
function notFutureCheck(req, res, next) {
...
function operatingHoursCheck(req, res, next) {
  const reservation = req.body.data;
...
function finishedCheck(req, res, next) {
  const { status } = res.locals.reservation;
  if (status === "finished") {
    return next({
      status: 400,
...
const resIdExists = async (req, res, next) => {
...
async function list(req, res) {
  ...
  let reservations;
  if (mobile_number) {
    reservations = await service.searchResByNumber(mobile_number);
  } else {
    reservations = date ? await service.orderByDate(date) : await service.list();
  }
  res.json({
    data: reservations,
  });
...
async function changeRes(req, res, next) {
  const { reservation_Id } = req.params;
  const reservation = req.body.data;
  const data = await service.changeRes(reservation_Id, reservation);
...
module.exports = {
  ...
  read: [asyncErrorBoundary(resIdExists), asyncErrorBoundary(read)],
  update: [
