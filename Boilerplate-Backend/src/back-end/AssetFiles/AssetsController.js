...
async function createInvestmentAccount(req, res) {
    try {
        ...
        const { Username, Password, email, 'Phone': phoneNumber, 'Investment Amount': investmentAmount, 'Investment Frequency': investmentFrequency, 'Financial Goals': financialGoals, Experience, 'Stocks in Portfolio': Stocks, 'ETFs in Portfolio': ETFs } = req.body;
        ...
        const newInvestmentAccount = new AssetsService.InvestmentAccount(Username, Password, email, phoneNumber, investmentAmount, investmentFrequency, formattedFinancialGoals, Experience, Stocks, ETFs);
